...
| D600 | 保持延迟工具与可恢复列表的请求前缀稳定 | **修订 D400 / D185 / ADR 0048 / ADR 0225 / ADR 0279（issue #913）：`ToolSearch` 写入规范字段 `details.addedToolNames`、兼容字段 `details.activated` 和顶层 `addedToolNames`；恢复按规范字段、activated、旧顶层字段、matches 的顺序采用第一个非空列表，持久化会提升历史标记但不覆盖有效规范证据。错误、占位、非法及已移出目录的名称仍被排除。动态可恢复列表与一次性恢复提示在发送时追加为运行时上下文，不得改写已发送的首部系统提示或历史；工具批次内的上下文要等匹配结果完整后再投影。权限、Task/TaskWait 和子代理生命周期不变。** | 生产者与恢复器曾读取不同字段，使未使用工具在下一回合消失；子代理结算还会改写首部系统提示，破坏 provider 前缀缓存。见 E2E-008a 与 E2E-008f。 |
...
## 2026-09-23 —— ToolSearch 与可恢复列表的缓存前缀稳定性（D600）

- `ToolSearch` 写入规范字段 `details.addedToolNames`、兼容字段
  `details.activated` 和顶层 `addedToolNames`。恢复按规范字段、activated、
  旧顶层字段、matches 的顺序采用第一个非空列表；转录和恢复的委托历史会
  提升历史标记，但不会覆盖有效规范证据。
- 可恢复列表更新与一次性恢复提示作为尾部运行时上下文发送，不改写首部系统
  提示或既有历史。工具批次内产生的上下文会等匹配结果完整后再投影，避免
  provider 适配器合成重复工具结果。
- 决策 D600 修订 D400 / ADR 0225 / ADR 0279。见 E2E-008a 与 E2E-008f。
