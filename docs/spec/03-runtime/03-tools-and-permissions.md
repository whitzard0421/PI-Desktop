...
session context. Successful `ToolSearch` results use the first non-empty
marker in this order: canonical `details.addedToolNames`,
`details.activated`, legacy top-level `addedToolNames`, then
`details.matches`. Successful deferred-tool results contribute their own
names. Unused activations stay restored while that
evidence remains. Failed rows, interrupted or missing result placeholders, and
assistant/user prose are ignored. Restored names must still be in the current
mode's deferred catalog. Transcript and resumed-delegate persistence promotes
historical markers to canonical `details.addedToolNames` without replacing an
existing valid canonical list. The host permission,
