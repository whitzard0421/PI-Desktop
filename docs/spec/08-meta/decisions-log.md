...
- `ToolSearch` writes `details.activated` and `addedToolNames`. Restore reads
  both plus `details.matches`, including unused activations, and persistence
  converts `activated` into `addedToolNames`. Invalid, failed, placeholder, and
