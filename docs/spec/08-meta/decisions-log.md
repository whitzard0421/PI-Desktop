...
| D400 | Restore deferred tools from effective session context | *(amended by D600)* **Amend D185 / ADR 0048: before each new prompt and after a mode switch, clear the in-memory deferred activation set, then restore names from successful `ToolSearch` results (`addedToolNames`) and successful deferred-tool results in the effective `buildSessionContext` projection. Keep only names still in the current deferred catalog and mode; ignore errors, interrupted or missing-result placeholders, and assistant/user prose. No host permission or workspace boundary changes.** | Clearing activations while retaining their successful transcript markers left the model able to see a capability that was absent from the next provider schema. Reconstructing only from effective successful evidence keeps the provider request coherent without parsing prose or reviving stale or disallowed tools. See ADR 0225 and E2E-008a. |
| D600 | Preserve request prefix for deferred tools and resumable lists | **Amend D400 / D185 / ADR 0048 / ADR 0225 / ADR 0279 (issue #913): `ToolSearch` emits canonical `details.addedToolNames`, compatibility `details.activated`, and top-level `addedToolNames`; restore takes the first non-empty list from canonical, activated, legacy top-level, then matches, and persistence promotes historical markers without replacing canonical evidence. Failed, placeholder, invalid, and off-catalog names stay excluded. Dynamic resumable-list text and one-shot recovery nudges are append-only runtime-context messages on the wire (internally owned system rows) and must not rewrite the already emitted head system prompt or history. `prepareNextTurn` keeps live system deltas in place when the non-system conversation is a content-equivalent extension, including repeated projections of the same checkpoint. A genuine rewrite retains only the current reusable snapshot and keeps recovery nudges separately removable. No permission, Task/TaskWait, or child-lifecycle change.** | The live producer wrote `activated` while restore read only `addedToolNames`, so unused tools vanished on the next user turn; child settlement rewrote the head system prompt and broke provider prefix cache. See E2E-008a and E2E-008f. |
...
## Cache-prefix stability for ToolSearch and resumable lists (D600)

- `ToolSearch` writes canonical `details.addedToolNames`, compatibility
  `details.activated`, and top-level `addedToolNames`. Restore takes the first
  non-empty list from canonical, activated, legacy top-level, then matches.
  Transcript and resumed-delegate persistence promotes historical markers but
  never overrides valid canonical evidence. Invalid, failed, placeholder, and
  off-catalog names stay excluded.
- Resumable-list updates and one-shot recovery nudges append trailing runtime
  context. Their internal system rows are projected before serialization so
  provider folding cannot rewrite the head system prompt or prior history.
- Decision D600 amends D400 / ADR 0225 / ADR 0279. See E2E-008a and E2E-008f.
